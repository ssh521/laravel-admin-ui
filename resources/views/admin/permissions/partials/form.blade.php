    ...
    <div class="mt-6 md:col-span-8 md:mt-0">
        <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
            <div class="sm:col-span-4">
                <label for="name" class="{{ $labelClass }}">{{ __('Permission Name') }} <span class="{{ $requiredClass }}" aria-hidden="true">*</span></label>
                <div class="mt-2">
                    <x-laravel-admin::admin.form-input id="name" name="name" value="{{ old('name', $permission?->name) }}" autocomplete="name" placeholder="{{ __('Enter permission name') }}" class="w-full" required />
                </div>
                @if ($errors->has('name'))
