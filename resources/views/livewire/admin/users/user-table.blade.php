<div class="mx-auto w-full max-w-5xl bg-white px-2 py-2 dark:bg-gray-900">
    <div class="min-h-[560px] bg-white px-4 py-6 sm:px-6 lg:px-8 dark:bg-gray-900">
        ...
        <div class="mt-6 rounded-md border border-gray-200 bg-gray-50/60 p-3 dark:border-gray-700 dark:bg-gray-800/60">
            <form class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end" x-data x-init="$nextTick(() => $refs.searchInput?.focus())">
                ...
                @if($search)
                    <button wire:click="$set('search', '')" type="button" class="inline-flex h-10 cursor-pointer items-center justify-center rounded-md border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700">
                        {{ __('초기화') }}
                    </button>
                @endif

                <button type="button" class="inline-flex h-10 cursor-pointer items-center justify-center rounded-md bg-indigo-600 px-4 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 dark:bg-indigo-500 dark:hover:bg-indigo-400">
                    {{ __('검색') }}
                </button>
            </form>
        </div>
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="min-w-0">
                                                    <div class="font-medium text-gray-900 dark:text-white">
                                                        @can('view', $user)
                                                            <button type="button" wire:click="showUserDetailModal({{ $user->id }})" class="cursor-pointer text-left font-medium text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                                                {{ $user->name }}
                                                            </button>
                                                        @else
                                                            {{ $user->name }}
