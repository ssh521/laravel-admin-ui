...
503 유지보수 화면은 외부 CSS·JavaScript·이미지 없이 렌더링되는 독립형 HTML이며, 모바일 화면과 safe area, 감소된 모션 환경을 지원하고 15초마다 자동으로 다시 연결을 시도합니다. 패키지 기본 화면을 바로 사용하려면 다음처럼 실행합니다.

```bash
php artisan down --render="laravel-admin::errors.503"
```

앱 이름·로고·문구를 서비스별로 변경하려면 먼저 호스트 앱의 `resources/views/errors/503.blade.php`로 퍼블리시한 뒤 수정합니다. 이미 같은 파일이 있으면 덮어쓰지 않으며, 의도적으로 교체할 때만 `--force`를 사용합니다.
...
503은 사이트 전체의 배포 상태이므로 패키지가 모든 503 응답을 자동으로 가로채지 않습니다. API 응답과 호스트 앱의 브랜드 화면은 각 앱이 소유합니다.
